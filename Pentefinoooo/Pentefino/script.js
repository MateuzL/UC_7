...
// =====================================================
const quiz = [
  { question: "Compartilhar pentes e bonés com os colegas?", correct: "no", success: "Isso! Objetos pessoais devem ser de uso individual.", error: "Quase! Pentes, bonés e acessórios devem ser de uso individual." },
  { question: "Avisar um adulto ao perceber muita coceira?", correct: "yes", success: "Muito bem! Pedir ajuda é uma atitude de cuidado.", error: "O melhor é avisar um adulto responsável para receber orientação adequada." },
  { question: "Zombar de um colega que está com piolho?", correct: "no", success: "Perfeito! Informação e respeito ajudam a combater o preconceito.", error: "Não pode. Pediculose não é motivo para vergonha ou brincadeira." },
  { question: "Usar o próprio pente e cuidar dos objetos pessoais?", correct: "yes", success: "Muito bem! Cuidar dos próprios objetos faz parte da prevenção.", error: "Usar os próprios objetos é uma atitude de cuidado." },
  { question: "Esconder dos responsáveis quando perceber sinais no couro cabeludo?", correct: "no", success: "Certo! Conversar com um adulto ajuda a receber orientação sem vergonha.", error: "Não é preciso esconder. O melhor é conversar com um adulto responsável." },
  { question: "Conversar sobre prevenção sem culpar ou constranger alguém?", correct: "yes", success: "Excelente! Informação, acolhimento e respeito fazem parte da missão.", error: "A melhor atitude é conversar com respeito e buscar orientação de um adulto." }
];
...
const duoTurnText = document.querySelector("#duoTurnText");
const duoPlayerLabel = document.querySelector("#duoPlayerLabel");

function updateDuoScoreboard() {
  const total = duoScores[0] + duoScores[1];
  if (playerOneScore) playerOneScore.textContent = duoScores[0];
  if (playerTwoScore) playerTwoScore.textContent = duoScores[1];
  if (scoreText) scoreText.textContent = `${total} / ${quiz.length}`;
  if (duoTurnText) duoTurnText.textContent = `Vez do Jogador ${duoPlayer}`;
  if (duoPlayerLabel) duoPlayerLabel.textContent = `VEZ DO JOGADOR ${duoPlayer}`;
}
...
function resetQuizQuestion() {
  ...
  if (quizMissionNumber) quizMissionNumber.textContent = `RODADA ${String(quizIndex + 1).padStart(2, "0")}`;
  if (quizFeedback) {
    quizFeedback.textContent = `Conversem e escolham a resposta juntos. Jogador ${duoPlayer} responde nesta rodada.`;
    quizFeedback.className = "feedback";
  }
  if (quizNext) quizNext.hidden = true;
  quizAnswers.forEach((button) => { button.disabled = false; });
  updateDuoScoreboard();
  setReaction("quiz", "neutral", "O Oficial Morango está pronto para comemorar o trabalho da dupla!", "assets/personagens/morango.webp", "Oficial Morango");
}
...
quizAnswers.forEach((button) => button.addEventListener("click", () => {
  ...
  setReaction(
    "quiz",
    isCorrect ? "good" : "bad",
    isCorrect ? `Boa, Jogador ${duoPlayer}! A dupla está avançando.` : "Vamos aprender com essa rodada e seguir juntos!",
    isCorrect ? "assets/personagens/morango.webp" : "assets/personagens/marinheiro.webp",
    isCorrect ? "Oficial Morango comemorando" : "Marinheiro Pente Fino orientando"
  );
  updateDuoScoreboard();
  ...
  if (quizNext) {
    ...
    quizNext.textContent = quizIndex === quiz.length - 1 ? "Ver resultado da dupla" : `Passar para Jogador ${duoPlayer === 1 ? 2 : 1}`;
  }
}));

quizNext?.addEventListener("click", () => {
  if (quizIndex < quiz.length - 1) {
    duoPlayer = duoPlayer === 1 ? 2 : 1;
    quizIndex++;
    resetQuizQuestion();
    quizAnswers[0]?.focus();
    return;
  ...
  quizQuestion.textContent = "Dupla do Pente Fino concluída!";
  if (quizFeedback) {
    quizFeedback.textContent = `A dupla marcou ${quizScore} de ${quiz.length}. O objetivo é aprender, cooperar, cuidar e respeitar.`;
    quizFeedback.className = "feedback good";
  }
  if (quizMissionNumber) quizMissionNumber.textContent = "RESULTADO";
  if (duoTurnText) duoTurnText.textContent = "Missão concluída!";
